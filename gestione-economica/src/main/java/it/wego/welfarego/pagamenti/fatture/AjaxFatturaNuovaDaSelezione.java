...
public class AjaxFatturaNuovaDaSelezione extends AbstractAjaxController {
     ...
     * ({@link PaiInterventoMese}) per creare le voci della fattura. Come chiave
...
//        try {
        ...
        List<PaiInterventoMese> interventi = cercaPaiInterventoMese(jsoIdPaiIntervento, jsoPeriodoRicerca, getEntityManager(), request);
        ...
        Iterable<Map> data = Iterables.transform(Multimaps.index(interventi, new Function<PaiInterventoMese, String>() {
            ...
            public void transformToMap(Map.Entry<String, Collection<PaiInterventoMese>> entry) {
                ...
                put("tipo_servizio", paiInterventoMeseCampione.getPaiIntervento().getTipologiaIntervento().getDesTipint());
...
//              
                ...
                BigDecimal quantita = BigDecimal.ZERO, importoSenzaIva = BigDecimal.ZERO,importoScontato = BigDecimal.ZERO,variazione = BigDecimal.ZERO, contributo = BigDecimal.ZERO;
                for (PaiInterventoMese paiInterventoMese : paiInterventoMeseList) {
                	BigDecimal costoStandard = getImportoStandard(paiInterventoMese.getPaiIntervento(), getEntityManager());
                	BigDecimal importoStandardScontato = costoStandard.multiply(percentualeRiduzione).setScale(2, RoundingMode.DOWN);
                    ...
                    importoScontato = importoScontato.add(paiInterventoMese.getBdgConsQta().multiply(importoStandardScontato).setScale(2, RoundingMode.HALF_UP));
                    if (paiInterventoMese.getBdgConsVar() != null) {
                        variazione = variazione.add(paiInterventoMese.getBdgConsVar());
                    }
// calcolo il contributo ??? coloni dice che non serve a nulla.. verificare. //TODO
                  /*  if (paiInterventoMese.getPaiIntervento().getTipologiaIntervento().getImpStdEntr() != null) {
                        contributo = contributo.add(paiInterventoMese.getPaiIntervento().getTipologiaIntervento().getImpStdEntr());
                        contributo = contributo.multiply(quantita);
                    }*/
                }
                ...
                put("importo_unitario", paiInterventoMeseCampione.getPaiIntervento().getTipologiaIntervento().getImpStdCosto().toString());
//                put("importo_unitario", paiInterventoMeseCampione.getPaiIntervento().getTipologiaIntervento().getImpStdCosto().multiply(percentualeRiduzione).toString());
                ...
                ParametriIndata ipAliquotaIva = paiInterventoMeseCampione.getPaiIntervento().getTipologiaIntervento().getIpAliquotaIva();
                ...
                put("totaleVariazioniConIva", variazione.multiply(ipAliquotaIva.getDecimalPercentageParamAsDecimal()).toString());
...
//            if (paiInterventoMese.getPaiIntervento().getIdParamUniMis() != null) {
//                String jsoId = this.creaIdPaiInterventoMese(paiInterventoMese.getPaiInterventoMesePK());
//                Map<String, String> jso = dataMap.get(jsoId);
//                if (jso != null) {
//                    jso.put("quantita", paiInterventoMese.getBdgConsQta().add(new BigDecimal(jso.get("quantita"))).toString());
//                    jso.put("importo_dovuto", paiInterventoMese.getBdgConsEur().add(new BigDecimal(jso.get("importo_dovuto"))).toString());
//                    if (paiInterventoMese.getBdgConsVar() != null) {
//                        jso.put("variazione_straordinaria", paiInterventoMese.getBdgConsVar().add(new BigDecimal(jso.get("variazione_straordinaria"))).toString());
//                    }
//                } else {
//                    jso = Maps.newHashMap();
//                    dataMap.put(jsoId, jso);
//                    jso.put("id", jsoId);
//                    jso.put("id_fattura", jsoIdPaiIntervento.toString());
...
//                    jso.put("quantita", paiInterventoMese.getBdgConsQta().toString());
////                    BigDecimal importoUnitario = Objects.firstNonNull(paiInterventoMese.getPaiIntervento().getTipologiaIntervento().getImpStdEntr(), BigDecimal.ZERO)
////                            .multiply(BigDecimal.valueOf(100).subtract(paiInterventoMese.getPaiIntervento().getPai().getIdParamFascia().getDecimalParam()))
////                            .divide(BigDecimal.valueOf(100));;
//
////                    double importoUnitario = intervento.getPaiIntervento().getTipologiaIntervento().getImpStdEntr() == null ? 0d : intervento.getPaiIntervento().getTipologiaIntervento().getImpStdEntr().doubleValue();
////                    importoUnitario = importoUnitario * (100 - intervento.getPaiIntervento().getPai().getIdParamFascia().getDecimalParam().doubleValue()) / 100;
//                    jso.put("importo_unitario", paiInterventoMese.getPaiIntervento().getTipologiaIntervento().getImpStdCosto().toString());
////                    jso.put("riduzione", JSONObject.NULL);
////                    jso.put("aumento", JSONObject.NULL);
//                    jso.put("variazione_straordinaria", Objects.firstNonNull(paiInterventoMese.getBdgConsVar(), BigDecimal.ZERO).toString());
//                    jso.put("mese", String.valueOf(paiInterventoMese.getPaiInterventoMesePK().getMeseEff()));
//
//                    jso.put("importo_dovuto", paiInterventoMese.getBdgConsEur().toString());
//                    jso.put("importoSenzaIva", paiInterventoMese.getBdgConsEur().toString());
//                    jso.put("importoConIva", paiInterventoMese.getBdgConsEur().multiply(
//                            Objects.firstNonNull(paiInterventoMese.getPaiIntervento().getTipologiaIntervento().getIpAliquotaIva(),
//                            new ParametriIndataDao(getEntityManager()).findOneByTipParamCodParam(TipologiaParametri.ALIQUOTA_IVA, Parametri.ALIQUOTA_IVA_DEFAULT)).getDecimalPercentageParamAsDecimal()).toString());
//                }//else
////                jso.put("importo_dovuto", (intervento.getPaiIntervento().getTipologiaIntervento().getImpStdEntr().doubleValue() * (Double) jso.get("quantita")));
//            }//if
//        }//for
//        if (jso != null) {
//            jsa.put(jso);
...
//        this.mandaRisposta(response, jsa, entityManager);
        JsonBuilder.newInstance().withWriter(response.getWriter()).withData(data).buildStoreResponse();
//        } catch (Exception e) {
//            jsonBuilder.withError(e);
...
//        }
    ...
    @Transactional
    public void salvaVociFattura(ResourceRequest request, ResourceResponse response, PortletSession session) throws IOException, JSONException, ParseException, JAXBException, Exception {
...
//            t.begin();
        for (int i = 0; i < dati.length(); i++) {
            ...
            if (jsoInterventoMese.has("aumento") && !JSONObject.NULL.equals(jsoInterventoMese.get("aumento"))) {
                aumentoOrig = interventoMese.getIdFattDettaglio().getAumento() == null ? 0 : interventoMese.getIdFattDettaglio().getAumento().doubleValue();
                interventoMese.getIdFattDettaglio().setAumento((new BigDecimal(((Number) jsoInterventoMese.get("aumento")).doubleValue())).multiply(fascia));
            }//if
            if (jsoInterventoMese.has("riduzione") && !JSONObject.NULL.equals(jsoInterventoMese.get("riduzione"))) {
                riduzioneOrig = interventoMese.getIdFattDettaglio().getRiduzione() == null ? 0 : interventoMese.getIdFattDettaglio().getRiduzione().doubleValue();
                interventoMese.getIdFattDettaglio().setRiduzione(new BigDecimal(((Number) jsoInterventoMese.get("riduzione")).doubleValue()).multiply(fascia));
            }//if
//                entityManager.merge(interventoMese.getIdFattDettaglio()); should be unnecessary to merge
            ...
            for (PaiInterventoMese paiInterventoMese : interventiMese) {
                ...
                double modificaAumento = aumentoOrig - (interventoMese.getIdFattDettaglio().getAumento() == null ? 0 : interventoMese.getIdFattDettaglio().getAumento().doubleValue());
                double modificaRiduzione = riduzioneOrig - (interventoMese.getIdFattDettaglio().getRiduzione() == null ? 0 : interventoMese.getIdFattDettaglio().getRiduzione().doubleValue());
                cdg.setImpComplUsingFascia(new BigDecimal((cdg.getImpCompl().doubleValue() - modificaAumento + modificaRiduzione) * proporzione));
                getEntityManager().persist(cdg);

                PaiEvento evento = Pratica.serializePaiEvento(paiInterventoMese.getPaiIntervento().getPai().getAnagrafeSoc(), paiInterventoMese.getPaiIntervento().getPai(), paiInterventoMese.getPaiIntervento(), "Inserimento Pai intervento mese in fattura", utente);
...
//                            try
//                            {
//                                variazione = Double.parseDouble(datoSpecificoDetermina.getValCampo());
//                            }//try
...
//                            entityManager.persist(datoSpecificoDetermina);
//                        }//if
            }//for
...
//        EntityManager entityManager = Connection.getEntityManager();
     ...
     * modificate dall'utente rispetto a quelle già presente nella base di
     * dati.
     *
     * @param request Richiesta del browser contenente la chiave del {@link
     * PaiIntervento} e il Codice fiscale della persona.
     * @param response Risposta al browser
     * @throws IOException In caso di problemi durante la scrittura del mesaggio
     * di risposta.
     * @throws JSONException In caso di problemi durante la formazione della
    ...
    @Transactional
    public void salvaQuoteObbligatiFatturazioni(ResourceRequest request, ResourceResponse response) throws IOException, JSONException {
//        EntityManager entityManager = Connection.getEntityManager();
...
//        jsa.put(jso);
//        this.mandaRisposta(response, jsa, null);
        JsonBuilder.newInstance().withWriter(response.getWriter()).withData(Collections.singletonList(intervento)).withTransformer(new JsonMapTransformer<PaiIntervento>() {
            @Override
...
//                    BigDecimal riduzioneFascia = BigDecimal.ZERO;
//                    ParametriIndata pin = interventoMese.getIdParamFascia();
//                    if (pin != null) {
//                        riduzioneFascia = pin.getDecimalParam();
//                    } 
//                    riduzioneFascia = new BigDecimal("100").subtract(riduzioneFascia).divide(new BigDecimal("100"));
////            BigDecimal.add(interventoMese.getBdgPrevQta().doubleValue() * interventoMese.getPaiIntervento().getTipologiaIntervento().getImpStdSpesa().doubleValue());
////                    contributo = contributo.add(interventoMese.getBdgPrevEur()); //TODO check variazioni std
//                    contributo = contributo.add(interventoMese.getBdgPrevQta().multiply(interventoMese.getPaiIntervento().getTipologiaIntervento().getImpStdSpesa()).multiply(riduzioneFascia)); //TODO era cosi' nell'originale . . ma non ha senso . . 
//                }//for

//                put("contributo", contributo);
                
                //17/08/2017 Modificato modo per reperite l'aliquota IVA. Il modo originela è palesement errato.
                for (PaiInterventoMese intMese : interventiMese)
                {
                	if (intMese.getPaiIntervento().getTipologiaIntervento().getIpAliquotaIva() != null)
                	{
                		put("iva", intMese.getPaiIntervento().getTipologiaIntervento().getIpAliquotaIva().getIdParamIndata());
                		break;
                	}
                }
//                if(paiIntervento.getTipologiaIntervento().getIpAliquotaIva()!=null){
//                	put("iva",paiIntervento.getTipologiaIntervento().getIpAliquotaIva().getIdParamIndata());
//                }
//                else{
//                put("iva", 326); //id nella base di dati dell'IVA ordinaria TODO da trovare il valore in mniera più intelligente
//                }
//                put("importo_iva", JSONObject.NULL);
                put("bollo", 0d);
//                put("totale_periodo", JSONObject.NULL);
//                jso.put("causale", JSONObject.NULL);
                if (paiIntervento.isDsPrimoEventoVita()) {
                    put("note", "in quanto primo intervento vita, la prima settimana non deve essere fatturata");
                } else {
//                    jso.put("note", JSONObject.NULL);
                }
                put("scadenza", DateTimeFormat.forPattern("dd/MM/yyyy").print(DateTime.now().monthOfYear().addToCopy(1)));
...
//                jso.put("modalita_pagamento", JSONObject.NULL);
     ...
     * collegate allo stesso {@link PaiIntervento} che sono state generate ma
     * non ancora emesse.
     *
     * @param request Richiesta del browser contenente la chiave di ricerca per
     * {@link PaiIntervento}).
    ...
    public void cercaMesiPrecedentiFatturazioni(ResourceRequest request, ResourceResponse response) throws IOException, JSONException {
//        EntityManager entityManager = Connection.getEntityManager();
    ...
    @Transactional
    public void salvaMesiPrecedentiFatturazioni(ResourceRequest request, ResourceResponse response) throws IOException, JSONException {
//        EntityManager entityManager = Connection.getEntityManager();
        JSONArray jsaDati = new JSONArray(request.getParameter("data"));

        if (((JSONObject) jsaDati.get(0)) != null && ((String) ((JSONObject) jsaDati.get(0)).get("id_fattura")) != null && !((String) ((JSONObject) jsaDati.get(0)).get("id_fattura")).equals("")) {
...
//        synchronized (entityManager) {
            ...
            //f.setFatturaList(fatturePrecedenti);
//            EntityTransaction t = entityManager.getTransaction();
//            t.begin();
...
//        }//synchronized
    ...
    @Transactional
    public void salvaFattura(ResourceRequest request, ResourceResponse response) throws IOException, JSONException, ParseException {
//        EntityManager entityManager = Connection.getEntityManager();
        JsonBuilder jsonBuilder = JsonBuilder.newInstance().withWriter(response.getWriter());
//        try {
...
//            fattura.setScadenza(FORMATO_DATA_EXT_JS.parse((String) jsoFattura.get("scadenza")));
        ...
        ParametriIndata paramIva = (ParametriIndata) getEntityManager().createNamedQuery("ParametriIndata.findByIdParamIndata").setParameter("idParamIndata", jsoFattura.get("iva")).getSingleResult();
        ...
        List<PaiInterventoMese> interventi = this.cercaPaiInterventoMese(jsoId, jsoPeriodoRicerca, getEntityManager(), request);
        ...
        fattura.setIdParamFascia((ParametriIndata) getEntityManager().createNamedQuery("ParametriIndata.findByIdParamIndata").setParameter("idParamIndata", jsoId.get("idFascia")).getSingleResult());
...
//        ArrayList<FatturaDettaglio> dettagli = new ArrayList<FatturaDettaglio>();
//        ArrayList<PaiCdg> listaCdg = new ArrayList<PaiCdg>();
//        String id = null;
//        FatturaDettaglio dettaglio = null;
        fattura.setFatturaDettaglioList(Lists.newArrayList(Iterables.transform(Multimaps.index(interventi, new Function<PaiInterventoMese, String>() {
            public String apply(PaiInterventoMese paiInterventoMese) {
                return creaIdPaiInterventoMese(paiInterventoMese.getPaiInterventoMesePK());
            }
        }).asMap().entrySet(), new Function<Map.Entry<String, Collection<PaiInterventoMese>>, FatturaDettaglio>() {
            public FatturaDettaglio apply(Map.Entry<String, Collection<PaiInterventoMese>> entry) {
//                String idInterventoMese = entry.getKey();
                ...
                fatturaDettaglio.setAnnoEff((int) paiInterventoMeseCampione.getPaiInterventoMesePK().getAnnoEff());
                fatturaDettaglio.setCausVar(paiInterventoMeseCampione.getCausVar());
                fatturaDettaglio.setCodTipint(paiInterventoMeseCampione.getPaiIntervento().getTipologiaIntervento());
                fatturaDettaglio.setIdFatt(fattura);
                fatturaDettaglio.setIdParamUnitaMisura(paiInterventoMeseCampione.getPaiIntervento().getIdParamUniMis());
                ...
                for (PaiInterventoMese paiInterventoMese : entry.getValue()) {
                	//modifica importante in data 04/04/ faccio in modo di scontare ogni singolo PIM.
                	   BigDecimal fascia = BigDecimal.ZERO;
                       if (paiInterventoMeseCampione.getIdParamFascia() != null) {
                           fascia = paiInterventoMeseCampione.getIdParamFascia().getDecimalParam();
                       }
                    fascia = (new BigDecimal("100").subtract(fascia)).divide(new BigDecimal("100"));
                    BigDecimal importoStandard = getImportoStandard(paiInterventoMese.getPaiIntervento(), getEntityManager());
                    BigDecimal daScontare = importoStandard.multiply(fascia).setScale(2,RoundingMode.DOWN);
                    ...
                    if (paiInterventoMese.getBdgConsQta() != null) {
                        fatturaDettaglio.setQtInputata(fatturaDettaglio.getQtInputata().add(paiInterventoMese.getBdgConsQta()));
                    }
                    if (paiInterventoMese.getBdgConsVar() != null) {
                        fatturaDettaglio.setVarStraord(fatturaDettaglio.getVarStraord().add(paiInterventoMese.getBdgConsVar()));
                    }
...
//                        new BigDecimal(intervento.getBdgConsQta().doubleValue() * intervento.getPaiIntervento().getTipologiaIntervento().getImpStdEntr().doubleValue()));
                fatturaDettaglio.setMeseEff((int) paiInterventoMeseCampione.getPaiInterventoMesePK().getMeseEff());
                ...
                for (PaiInterventoMese paiInterventoMese : entry.getValue()) {
                    if (paiInterventoMese.getPaiCdg() == null) {
                        ...
                        cdg.setImpStd(getImportoStandard(paiInterventoMese.getPaiIntervento(), getEntityManager()));
                    ...
                    paiInterventoMese.getPaiCdg()
                            .setImpComplUsingFascia(paiInterventoMese.getBdgConsEur().add(MoreObjects.firstNonNull(paiInterventoMese.getBdgConsVar(), BigDecimal.ZERO)));
...
//            String idInterventoMese = this.creaIdPaiInterventoMese(intervento.getPaiInterventoMesePK());
//            if (idInterventoMese.equals(id)) {
//                dettaglio.setImporto(new BigDecimal(dettaglio.getImporto().doubleValue() + (intervento.getBdgConsQta().doubleValue() * intervento.getPaiIntervento().getTipologiaIntervento().getImpStdEntr().doubleValue())));
//                dettaglio.setQtInputata(new BigDecimal(dettaglio.getQtInputata().doubleValue() + intervento.getBdgConsQta().doubleValue()));
//                if (dettaglio.getVarStraord() != null) {
//                    dettaglio.setVarStraord(new BigDecimal(dettaglio.getVarStraord().doubleValue() + intervento.getBdgConsVar().doubleValue()));
//                }//if
//            }//if
//            else {
//                if (dettaglio != null) {
...
//                }//if
//                id = idInterventoMese;
//                dettaglio = new FatturaDettaglio();
//                dettaglio.setAumento(BigDecimal.ZERO);
//                dettaglio.setAnnoEff((int) intervento.getPaiInterventoMesePK().getAnnoEff());
//                dettaglio.setCausVar(intervento.getCausVar());
//                dettaglio.setCodTipint(intervento.getPaiIntervento().getTipologiaIntervento());
//                dettaglio.setIdFatt(fattura);
//                dettaglio.setIdParamUnitaMisura(intervento.getPaiIntervento().getIdParamUniMis());
//                dettaglio.setImporto(new BigDecimal(intervento.getBdgConsQta().doubleValue() * intervento.getPaiIntervento().getTipologiaIntervento().getImpStdEntr().doubleValue()));
//                dettaglio.setMeseEff((int) intervento.getPaiInterventoMesePK().getMeseEff());
//                dettaglio.setQtInputata(intervento.getBdgConsQta());
//                dettaglio.setRiduzione(BigDecimal.ZERO);
//                dettaglio.setTimbro(new Date());
//                dettaglio.setVarStraord(intervento.getBdgConsVar());
//            }//else
//            intervento.setIdFattDettaglio(dettaglio);
//            PaiCdg cdg = intervento.getPaiCdg();
//            if (cdg == null) {
//                //se non esistono righe in controllo di gestione, le creo
//                cdg = new PaiCdg();
//                PaiCdgPK cdgPK = new PaiCdgPK();
//                cdgPK.setCntTipint(intervento.getPaiInterventoMesePK().getCntTipint());
//                cdgPK.setCodPai(intervento.getPaiInterventoMesePK().getCodPai());
//                cdgPK.setCodTipint(intervento.getPaiInterventoMesePK().getCodTipint());
//                cdgPK.setCodAnno(intervento.getPaiInterventoMesePK().getAnno());
//                cdgPK.setAnnoEff(intervento.getPaiInterventoMesePK().getAnnoEff());
//                cdgPK.setMeseEff(intervento.getPaiInterventoMesePK().getMeseEff());
//                cdgPK.setCodImpe(intervento.getPaiInterventoMesePK().getCodImp());
//                cdg.setPaiCdgPK(cdgPK);
//                cdg.setPaiInterventoMese(intervento);
//                cdg.setCodAna(intervento.getPaiIntervento().getPai().getAnagrafeSoc());
//                cdg.setCodCap(intervento.getBudgetTipIntervento().getCodCap());
//                cdg.setCcele(intervento.getPaiIntervento().getTipologiaIntervento().getCcele());
//                cdg.setImpStd(intervento.getPaiIntervento().getTipologiaIntervento().getImpStdCosto());
//                cdg.setImpVar(intervento.getBdgConsVar());
//                cdg.setQtaPrev(intervento.getBdgPrevQta());
//                intervento.setPaiCdg(cdg);
//            }//if
//            cdg.setImpComplUsingFascia(new BigDecimal(intervento.getBdgConsEur().doubleValue() + (intervento.getBdgConsVar() == null ? 0 : intervento.getBdgConsVar().doubleValue())));
//            cdg.setQtaErog(intervento.getBdgConsQta());
////            listaCdg.add(cdg);
//        }//for
//        if (dettaglio != null) {
//            dettagli.add(dettaglio);
//        }//if
        //il salvataggio dei mesi precedenti verrà eventualmente realizzato con
        //una chiamata separata.
//        fattura.setFatturaDettaglioList(dettagli);
//            EntityTransaction t = entityManager.getTransaction();
//        synchronized (entityManager) {
//            t.begin();
        getEntityManager().persist(fattura);
//        for (PaiCdg cdg : listaCdg) {
//            getEntityManager().persist(cdg);
//        }//for
//            t.commit();
//        }//synchronized
...
//        JSONObject jsoId = new JSONObject(id);
//        String codPai=new JSONObject(id).getString("codPai");
//        CriteriaBuilder cb = entityManager.getCriteriaBuilder();
//        CriteriaQuery c = cb.createQuery();
//        Root r = c.from(PaiIntervento.class);
//        Predicate p = cb.equal(r.get("paiInterventoPK").get("codPai"), jsoId.get("codPai"));
//        c.where(p);
//        return (entityManager.createQuery(c).getResultList());
     ...
     * solamente i record che non sono stati ancora associati a qualche fattura
     * e che il {@link PaiIntervento} è nello stato di esecutività.
     *
     ...
     * risposta in formato JSON (non previsto)
     */
    private List<PaiInterventoMese> cercaPaiInterventoMese(JSONObject jsoIdPai, JSONObject jsoPeriodoRicerca, EntityManager entityManager, ResourceRequest request) throws JSONException {
        CriteriaBuilder cb = entityManager.getCriteriaBuilder();
        CriteriaQuery c = cb.createQuery();
        Root r = c.from(PaiInterventoMese.class);
        Join pi = r.join("paiIntervento", JoinType.INNER);
