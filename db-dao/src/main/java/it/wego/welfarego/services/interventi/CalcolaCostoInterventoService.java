...
public class CalcolaCostoInterventoService {
    ...
    public BigDecimal calcolaBdgPrevEur(Integer durMesi, Date dtAvvio, Date dtFine, BigDecimal quantita, Tariffa tariffa, TipologiaIntervento tipologiaIntervento) {
        BigDecimal costo;
        ...
        if (isPerMesi && isConStruttura) {
            costo = calcolaCostoInterventoConStrutturaPerDurata(quantita, durMesi, tipologiaIntervento, tariffa, dtAvvio);
    ...
    BigDecimal calcolaCostoInterventoConStrutturaPerDurata(BigDecimal quantita, Integer durata, TipologiaIntervento ti, Tariffa tariffa, Date dtAvvio) {
        BigDecimal costo = null;
        ...
        if (is_costo_forfettario(tariffa)) {
            ...
            if (is_quantita_mensile(ti)) {
                ...
                throw new RuntimeException("la tipologia inetervento non è giornaliera, ne settimanale ne mensile. Tipologia intervento: " + ti.getCodTipint() + " - " + ti.getDesTipint());
            }
        }
     ...
     * @return
     */
    BigDecimal calcolaCostoInterventoPerDurata(TipologiaIntervento ti, BigDecimal quantita, Integer durataAsInteger, Date dtAvvio) {
        ...
        if (is_quantita_mensile(ti) ||
                is_intervento_per_durata(ti) && !is_quantita_settimanale(ti)
                ) {
        ...
        LocalDate jodaDtAvvio = new LocalDate(dtAvvio);
            ...
            throw new RuntimeException("la tipologia intervento non è giornaliera, ne settimanale ne mensile. Tipologia intervento: " + ti.getCodTipint() + " - " + ti.getDesTipint());
        }
    ...
    BigDecimal calcolaCostoInterventoConStruttura(TipologiaIntervento ti, BigDecimal quantita, Date dal, Date al, Tariffa tariffa) {
        ...
        if (is_costo_forfettario(tariffa)) {
            ...
            if (is_quantita_mensile(ti)) {
                Integer numMesi = org.joda.time.Months.monthsBetween(new DateTime(dal), new DateTime(al).plusDays(1)).getMonths();
                ...
                Integer numGiorni = Days.daysBetween(new DateTime(dal).withZone(DateTimeZone.forID("Europe/Rome")), new DateTime(al).withZone(DateTimeZone.forID("Europe/Rome")).plusDays(1)).getDays();
