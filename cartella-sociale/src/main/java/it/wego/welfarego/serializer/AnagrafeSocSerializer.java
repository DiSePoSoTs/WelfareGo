 ...
 * @author giuseppe
 */
public class AnagrafeSocSerializer {

	private EntityManager em;

	public AnagrafeSocSerializer(EntityManager em) {
		this.em = em;
	}

	public AnagraficaBean anagrafeSocToAnagraficaBean(AnagrafeSoc anagrafe) {
		return anagrafeSocToAnagraficaBean(anagrafe, null);
	}

	public AnagraficaBean anagrafeSocToAnagraficaBean(AnagrafeSoc anagrafe, @Nullable Pai pai) {
		...
		if (anagrafe.getIdParamTipologiaResidenza() != null) {
			anagraficaBean.setAnagraficaTipologiaResidenza(
					anagrafe.getIdParamTipologiaResidenza().getIdParamIndata().toString());
		}
		...
		LiberatoriaDao ldao = new LiberatoriaDao(em);
		Liberatoria comuneL = ldao.getLiberatoriaByAssociazioneAndUser(1, anagrafe.getCodAna());
		if (comuneL != null) {
			anagraficaBean.setAnagraficaLiberatoria("on");
		}
		...
		anagraficaBean.setAnagraficaComuneDiNascitaDesc(
				anagrafe.getLuogoNascita().getCodCom() == null ? anagrafe.getLuogoNascita().getComuneStr()
						: anagrafe.getLuogoNascita().getCodCom());
		if (anagrafe.getIdParamPosAna() != null) {
			anagraficaBean
					.setAnagraficaPosizioneAnagrafica(String.valueOf(anagrafe.getIdParamPosAna().getIdParamIndata()));
		}
		...
		anagraficaBean.setAnagraficaProvinciaNascitaDesc(
				anagrafe.getLuogoNascita().getCodProv() == null ? anagrafe.getLuogoNascita().getProvinciaStr()
						: anagrafe.getLuogoNascita().getCodProv());
		...
		anagraficaBean
				.setAnagraficaCapResidenza(StringConversion.objectToString(anagrafe.getLuogoResidenza().getCap()));
		anagraficaBean
				.setAnagraficaCapDomicilio(StringConversion.objectToString(anagrafe.getLuogoDomicilio().getCap()));
	...
	public AnagrafeSoc serializeFamiglia(HttpServletRequest request) throws Exception {
		...
		if (request.getParameter("comuneResidenza").equals(ComuneConstants.COMUNE_TRIESTE) && comuneResidenza != null) {
			...
			ToponomasticaCivici toponomasticaCivici = toponomastica == null ? null
					: toponomastica.getToponomasticaCivici(request.getParameter("civicoResidenza"));
	 ...
	 * @throws Exception
	 * @deprecated use
	 *             requestParamsToAnagrafeSoc(WebUtils.getParametersMap(request))
	 *             instead
	 */
	@Deprecated
	...
	public AnagrafeSoc requestParamsToAnagrafeSoc(Map<String, String> params) throws ParseException {
		...
		// TODO: Lo forzo ad essere fisica. non è ancora prevista la gestione delle
		// persone giuridiche
		...
		if (params.get("anagraficaComuneResidenza") != null) {
			...
			if (comuneResidenza != null && !comuneResidenza.getToponomasticaList().isEmpty()) {
				...
				Preconditions.checkArgument(toponomastica != null, "via residenza %s non presente nello stradario",
						params.get("anagraficaViaResidenza"));
				anagrafe.getLuogoResidenza().setVia(toponomastica);
				if (!Strings.isNullOrEmpty(params.get("anagraficaCivicoResidenza"))) {
					ToponomasticaCivici toponomasticaCivici = toponomastica
							.getToponomasticaCivici(params.get("anagraficaCivicoResidenza"));
					Preconditions.checkArgument(toponomasticaCivici != null,
							"numero civico residenza %s non presente nello stradario",
							params.get("anagraficaCivicoResidenza"));
					anagrafe.getLuogoResidenza().setCivico(toponomasticaCivici);
				}
		...
		if (params.get("anagraficaComuneDomicilio") != null) {
			...
			if (comuneDomicilio != null && !comuneDomicilio.getToponomasticaList().isEmpty()) {
				...
				Preconditions.checkArgument(toponomastica != null, "via domicilio %s non presente nello stradario",
						params.get("anagraficaViaDomicilio"));
				anagrafe.getLuogoDomicilio().setVia(toponomastica);
				ToponomasticaCivici toponomasticaCivici = toponomastica
						.getToponomasticaCivici(params.get("anagraficaCivicoDomicilio"));
				Preconditions.checkArgument(toponomasticaCivici != null,
						"numero civico domicilio %s non presente nello stradario",
						params.get("anagraficaCivicoDomicilio"));
		...
		if (!Strings.isNullOrEmpty(tipologiaResidenzaStr)) {
			anagrafe.setIdParamTipologiaResidenza(
					parametriDao.findByIdParamIndata(Integer.parseInt(tipologiaResidenzaStr)));
		}

		if (!Strings.isNullOrEmpty(params.get("anagraficaPosizioneAnagrafica"))) {
			ParametriIndata posizioneAnagrafica = parametriDao
					.findByIdParamIndata(Integer.valueOf(params.get("anagraficaPosizioneAnagrafica")));
	...
	public CartellaSociale serializeAnagraficaCartella(HttpServletRequest request, AnagrafeSoc anagrafe, List<Pai> pai)
			throws ParseException {
		CartellaSociale cartella = new CartellaSociale();
		cartella.setCodAna(anagrafe.getCodAna());
		cartella.setDtApCs(
				StringConversion.stringToDate(request.getParameter("anagraficaDataAperturaCartella"), "dd/MM/yyyy"));
	...
	public AnagrafeSoc serializeCondizione(HttpServletRequest request) throws ParseException, Exception {
		...
		ParametriIndataDao parametriDao = new ParametriIndataDao(em);
		if (request.getParameter("condizioneStatoCivile") != null
				&& !"".equals(request.getParameter("condizioneStatoCivile"))) {
		...
		if (request.getParameter("condizioneCondizioneProfessionale") != null
				&& !"".equals(request.getParameter("condizioneCondizioneProfessionale"))) {
		...
		if (request.getParameter("condizioneAbitazione") != null
				&& !"".equals(request.getParameter("condizioneAbitazione"))) {
		...
		if (request.getParameter("condizioneFormaReddito") != null
				&& !"".equals(request.getParameter("condizioneFormaReddito"))) {
		...
		if (request.getParameter("condizioneTitoloStudio") != null
				&& !"".equals(request.getParameter("condizioneTitoloStudio"))) {
		...
		if (request.getParameter("condizioneCondizioneFamiliare") != null
				&& !"".equals(request.getParameter("condizioneCondizioneFamiliare"))) {
		...
		if (request.getParameter("condizioneStatoFisico") != null
				&& !"".equals(request.getParameter("condizioneStatoFisico"))) {
		...
		if (request.getParameter("condizioneRedditoMensile") != null
				&& !"".equals(request.getParameter("condizioneRedditoMensile"))) {
			a.setReddMens(new BigDecimal(request.getParameter("condizioneRedditoMensile").replace(',', '.')));
		} else {
			a.setReddMens(null);
		}
		if (request.getParameter("condizioneAccompagnamento") != null
				&& !"".equals(request.getParameter("condizioneAccompagnamento"))) {
		...
		a.setDataRichiestaAssegnoAccompagnamento(
				StringUtils.isBlank(value) ? null : StringConversion.stringToDate(value, "dd/MM/yyyy"));

		String condizioneInvaliditaCivile = Strings.emptyToNull(request.getParameter("condizioneInvaliditaCivile"));
		a.setPercInvCiv(condizioneInvaliditaCivile == null ? null
				: Short.valueOf(condizioneInvaliditaCivile.replaceAll("%", "")));
		return a;
	}
	 ...
	 * @return
	 * @deprecated use
	 *             VistaAnagrafeDao(em).allineaAnagraficaWegoDaAnagraficaComune(vistaAnagrafe);
	 */
	@Deprecated
	...
	public AnagraficaBean vistaAnagraficaToAnagraficaBean(VistaAnagrafe vistaAnagrafe) {
		VistaAnagrafeDao vistaAnagrafeDao = new VistaAnagrafeDao(em);
		AnagrafeSoc anagrafeSoc = vistaAnagrafeDao.anagrafeComune2AnagrafeWego(vistaAnagrafe);
		return anagrafeSocToAnagraficaBean(anagrafeSoc);
	}

	private void cleanFields(AnagrafeSoc anagrafe) {

		for (Luogo luogo : Arrays.asList(anagrafe.getLuogoNascita(), anagrafe.getLuogoDomicilio(),
				anagrafe.getLuogoResidenza())) {
			luogo.clear();
		}
