...
/**
 * @class wf.ux.VirtualKeyboard
 * @extends Ext.Component
 */ 
Ext.define('wf.ux.VirtualKeyboard', {
	/* ***** Create keyboards **************************************** */
	...
	onRender: function(el){
		log('rendering keyboard');
		this.callParent(arguments);
		this.initKeyboard(el.child('.x-component'));
	},
	
	initKeyboard : function(el){
		this.keyboardTarget.on('click', this.IESel, this);
		this.keyboardTarget.on('keyup', this.IESel, this);
		this.keyboardTarget.on('select', this.IESel, this);

		this.keyboard = el.createChild({
			tag: 'div',
			...
			this.languageSelector = new Ext.form.ComboBox({
				...
				listeners: {
					select: function(combo) {
						this.language = combo.getRawValue();
						this.buildKeys();
					},
					expand: function(){
		...
		if (!this.languageSelection){
			Ext.fly(this.ktbar.items.items[0].getEl()).setStyle({
				fontWeight: 'bold'
			});
		}
	...
	buildKeys: function() {		
		...
		for (var x = 0, hasdeadKey = false, lyt; lyt = this.Languages[this.language][x++];) {
			...
			var tbody = dh.append(table, {
				tag: 'tbody'
			});
			var tr = dh.append(tbody, {
				tag: 'tr'
			});
			...
			for (var y = 0, lkey; lkey = lyt[y++];) {
				...
				Ext.fly(td).on('mouseover', function(event, target) {
					if (!Ext.fly(target).hasCls('dead') && target.firstChild.nodeValue != "\xa0") Ext.fly(target).addCls('hover');
				});
				Ext.fly(td).on('mouseout', function(event, target) {
					if (!Ext.fly(target).hasCls('dead')) Ext.fly(target).removeCls(['hover', 'pressed']);
				});
				Ext.fly(td).on('mousedown', function(event, target) {
					if (!Ext.fly(target).hasCls('dead')) Ext.fly(target).addCls('pressed');
				});
				Ext.fly(td).on('mouseup', function(event, target) {
					if (!Ext.fly(target).hasCls('dead') && target.firstChild.nodeValue != "\xa0") Ext.fly(target).removeCls('pressed');
				});
				...
				switch (lkey[1]) {
					...
					case "Bksp":
						Ext.fly(td).on('click', function() {
							...
							var dom = this.keyboardTarget.dom;
							if(dom.readOnly)
								return;
							if (dom.setSelectionRange) {
								...
								if (srt < len) srt++;
								dom.value=(dom.value.substr(0, srt - 1) + dom.value.substr(len));
								...
								this.range.text = "";
							} else dom.value=(dom.value.substr(0, dom.value.length - 1));
							if (this.shift) this.keyModify("Shift");
							if (this.alternate) this.keyModify("AltGr");
							return true;
							this.keyboardTarget.focus();
							var value=this.keyboardTarget.getValue();
			...
			for (var y = 0; y < tds.getCount(); y++) {
				...
				switch (lkey[1]) {
					...
					case "Bksp":
						break;
					default:
						var cchar = lkey[vchar + ((this.alternate && lkey.length == 4) ? 2 : 0)];
						if (type)
							td.update(cchar);
						if (this.deadKeysOn) {
							if (this.dead) {
								...
								for (var z = 0; z < this.deadKey[this.dead].length; z++){
									if (cchar == this.deadKey[this.dead][z][0]) {
										isTarget = true;
										break;
									}
								}
				...
				if (y == tds.getCount() - 1 && tds.getCount() > this.keyCenter) td.addCls('last');
			}
		}
	},
	...
	setKeyboardTarget:function(keyboardTarget){
		log('changing vk target : '+keyboardTarget);
